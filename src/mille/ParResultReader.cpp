...
namespace
{
    using millepede::ParResultEntry;
    ...
    void ResultReader::read()
    {
        ...
        if (ifile.is_open())
        {
            ...
            while (std::getline(ifile, buffer))
            {
                ...
                if (result.has_value())
                {
                    par_results_.emplace(result->par_id, result.value());
                }
            }
        }
        else
        {
            std::println("ERROR: parameter file {:?} cannot be read", filename_);
        }
    }
    void ResultReader::print()
    {
        std::println("{}",
                     std::views::join_with(par_results_ | std::views::values |
                                               std::views::transform([](const auto& val) -> std::string
